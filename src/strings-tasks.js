...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .reduce((sum, x) => sum + str.charCodeAt(str.indexOf(x)), 0);
}
...
function formatTime(minutes, seconds) {
  ...
  if (min.length > 1 && sec.length > 1) return `${min}:${sec}`;
  if (min.length === 1 && sec.length === 1)
    return `${min.padStart(2, '0')}:${sec.padStart(2, '0')}`;
  if (min.length === 1) return `${min.padStart(2, '0')}:${sec}`;
...
function findLongestWord(sentence) {
  return sentence
    ...
    .filter(
      (x) =>
        x.length ===
        sentence.split(' ').sort((a, b) => a.length - b.length)[
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    res += !origAlphabet.includes(str[i])
      ? str[i]
      : rotAlphabet[origAlphabet.indexOf(str[i])];
  }
  return res;
}
